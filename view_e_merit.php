...
$nextLink = "view_certificate.php?id=$submission_id&user=$user_id";
?>

<!DOCTYPE html>
<html>
<head>
    <title>View E-Merit Submission</title>
...
<div class="form-section">
    <h2 style="text-align:center; color:#2a2675;">E-Merit Form (Read-Only)</h2>
    <form class="form-grid">
        <label>Program Name:</label>
        <input type="text" value="<?= htmlspecialchars($formData['program_name'] ?? '') ?>" disabled>

        <label>Date:</label>
        <input type="date" value="<?= htmlspecialchars($formData['program_date'] ?? '') ?>" disabled>

        <label>Venue:</label>
        <input type="text" value="<?= htmlspecialchars($formData['venue'] ?? '') ?>" disabled>

        <label>Organizer:</label>
        <input type="text" value="<?= htmlspecialchars($formData['organizer'] ?? '') ?>" disabled>

        <label>Coupons:</label>
        <input type="number" value="<?= htmlspecialchars($formData['coupon'] ?? '') ?>" disabled>
    </form>

    <div class="form-buttons" style="text-align:center; margin-top:20px;">
        <a href="view_c_form.php?id=<?= $submission_id ?>&user=<?= $user_id ?>" class="button">← Back</a>
