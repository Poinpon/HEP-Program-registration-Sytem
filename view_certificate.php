...
$nextLink = "view_bookhall.php?id=$submission_id&user=$user_id";
?>

<!DOCTYPE html>
<html>
<head>
    <title>View Certificate Submission</title>
...
<div class="form-section">
    <h2 style="text-align:center; color:#2a2675;">Certificate Form (Read-Only)</h2>
    <form class="form-grid">
        <label>Association / Club:</label>
        <input type="text" value="<?= htmlspecialchars($formData['association'] ?? '') ?>" disabled>

        <label>Program Name:</label>
        <input type="text" value="<?= htmlspecialchars($formData['program_name'] ?? '') ?>" disabled>

        <label>Date:</label>
        <input type="date" value="<?= htmlspecialchars($formData['date'] ?? '') ?>" disabled>

        <label>Venue:</label>
        <input type="text" value="<?= htmlspecialchars($formData['venue'] ?? '') ?>" disabled>

        <label>Report Submission Date:</label>
        <input type="date" value="<?= htmlspecialchars($formData['report_date'] ?? '') ?>" disabled>
    </form>

    <div class="form-buttons" style="text-align:center; margin-top:20px;">
        <a href="view_e_merit.php?id=<?= $submission_id ?>&user=<?= $user_id ?>" class="button">← Back</a>
